    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with torch.no_grad():
            ...
            for x, y_v1, y_v2, bgm, v1_paths in tqdm.tqdm(
                duet_loader, desc="Evaluating DUET Test (FAD, with film basic embedding model)"
            ):
                x = x.to(device)
                y_v1 = y_v1.to(device)
                y_v2 = y_v2.to(device)
        ...
        print("\n--- DUET Test FAD Summary (film basic Embedding Model) ---")
    ...
    try:
        with torch.no_grad():
            ...
            for x, y_v1, v1_paths, bgm in tqdm.tqdm(
                solo_loader, desc="Evaluating SOLO Test (FAD, FILM BASIC)"
            ):
                x = x.to(device)
                y_v1 = y_v1.to(device)
                bgm = bgm.to(device)
        ...
        print("\n--- SOLO Test FAD Summary (FILM_BASIC) ---")
        print(f"vocal_fad: {vocal_fad:.4f}")
...
def save_first_test_outputs(unmix, encoder, decoder, device, test_loader, emb_map_test, save_dir="TEST_AUDIO_DEBUG_EMB_FILM"):
...
def save_first_test_outputs_solo(
    ...
    save_dir="TEST_AUDIO_DEBUG_SOLO_EMB_FILM", max_examples=10, sample_rate=44100
):
    import os
    import tqdm
    import torchaudio
