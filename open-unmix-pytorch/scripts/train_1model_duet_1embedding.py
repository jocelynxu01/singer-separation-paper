...
# -------------------------------------------------------------------------
# Utility: Load precomputed embeddings (v1 only)
# -------------------------------------------------------------------------
...
#         if entry is None:
#             raise FileNotFoundError(f"No embedding found for {file_path}")
...
#             raise FileNotFoundError(f"Embedding file missing for {file_path}: {embedding_path}")
...
def train(args, unmix, encoder, decoder, device, train_loader, optimizer, emb_map):
    ...
    for x, y_v1, y_v2, bgm, v1_paths in pbar:
        ...
        loss = singlesrc_neg_sisdr(y_hat_time, y_target).mean()
        loss.backward()
        optimizer.step()

        losses.update(loss.item(), y_target.size(0))
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with torch.no_grad():
            ...
            for x, y_v1, y_v2, bgm, v1_paths in tqdm.tqdm(
                duet_loader, desc="Evaluating DUET Test (FAD, with concat basic embedding model)"
            ):
                x = x.to(device)
                y_v1 = y_v1.to(device)
                y_v2 = y_v2.to(device)
        ...
        print("\n--- DUET Test FAD Summary (conct basic Embedding Model) ---")
    ...
    try:
        with torch.no_grad():
            ...
            for x, y_v1, v1_paths, bgm in tqdm.tqdm(
                solo_loader, desc="Evaluating SOLO Test (FAD, CONCAT BASIC)"
            ):
                x = x.to(device)
                y_v1 = y_v1.to(device)
                bgm = bgm.to(device)
        ...
        print("\n--- SOLO Test FAD Summary (CONCAT BASIC) ---")
        print(f"vocal_fad: {vocal_fad:.4f}")
...
def save_first_test_outputs(unmix, encoder, decoder, device, test_loader, emb_map_test, save_dir="TEST_AUDIO_DEBUG_EMB"):
...
def save_first_test_outputs_solo(
    ...
    save_dir="TEST_AUDIO_DEBUG_SOLO_EMB", max_examples=10, sample_rate=44100
):
    import os
    import tqdm
    import torchaudio
...
def main():
    ...
    unmix = model.OpenUnmix_Embedding(
    ...
    for epoch in tqdm.trange(1, args.epochs + 1, desc="Training epochs"):
        ...
        train_loss = train(args, unmix, encoder, decoder, device, train_loader, optimizer, emb_map_train)
        valid_loss = validate(args, unmix, encoder, decoder, device, valid_loader, emb_map_valid)
...
def main_test_with_embeddings():
    ...
    v, r, m, improv  = valid_for_test_duet_emb_fad(unmix, encoder, decoder, device, test_loader, emb_map_test)
