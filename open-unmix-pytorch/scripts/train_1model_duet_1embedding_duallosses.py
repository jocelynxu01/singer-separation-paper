    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with torch.no_grad():
            ...
            for x, y_v1, y_v2, bgm, v1_paths in tqdm.tqdm(
                duet_loader, desc="Evaluating DUET Test (FAD, with concat embedding model)"
            ):
                x = x.to(device)
                y_v1 = y_v1.to(device)
                y_v2 = y_v2.to(device)
        ...
        print("\n--- DUET Test FAD Summary (concat Embedding Model) ---")
...
def save_first_test_outputs(unmix, encoder, decoder, device, test_loader, emb_map_test, save_dir="TEST_AUDIO_DEBUG_EMB_DUELLOSSES0.2"):
